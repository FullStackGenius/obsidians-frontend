...
export type ActionState = {
  ...
  fieldValues?: Partial<any & { [key: string]: any }>; // for repopulating form
};

export default async function projectAction(
  prevState: ActionState,
  formData: FormData
): Promise<any> {
    console.log(formData,"formData")
     try {
    const res:any = await postForm("/api/project/create", formData);
    console.log(res,"res")

    if (!res?.success) {
      throw new Error(res?.message || "Project create failed");
    }
    ...
    return {
      success: false,
      errors: {
        _general: err instanceof Error ? err.message : "Something went wrong",
      },
    };
  }
//   try {
