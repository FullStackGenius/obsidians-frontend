...
const page = () => {
  ...
  const [projectSizes, setProjectSize] = useState(["sfsaf","weffwer"]);
  const [timeLines, setTimeLine] = useState(["sfsaf","weffwer"]);
  const [timeResources, setTimeResource] = useState(["sfsaf","weffwer"]);
  const [kpiss, setKpis] = useState(["sfsaf","weffwer"]);


  const addMoreProjectSize = () => {
    setProjectSize([...projectSizes, ""]);
  }
  ...
  const addTechnologyData = () => {
    setTechnology([...technologies, ""]);
  }


  const removeTechnologyData = (index) => {
    const techno = technologies.filter((_, id) => id !== index)
    setTechnology(techno);
  }

  const handleChangeTechnolgy = (value, index) => {
  ...
  const onRemoveProjectSize = (index) => {
    const update = projectSizes.filter((_, id) => id !== index);
    setProjectSize(update);
  }

  const handleChangeProjectSize = (value, index) => {
    const update = [...projectSizes];
    update[index] = value;
    setProjectSize(update);
  }

  const handleChangeTimeLine = (value, index) => {
    const update = [...timeLines];
    update[index] = value;
    setTimeLine(update);
  }

  const onRemoveTimeLine = (index) => {
    const update = timeLines.filter((_, id) => id !== index);
    setTimeLine(update);
  }

  const addMoreTimeLine = () => {
    setTimeLine([...timeLines, ""]);
  }

  const addMoreTimeResource = () => {
    setTimeResource([...timeResources, ""]);
  }

  const handleChangeTimeResource = (value, index) => {
    const update = [...timeResources];
    update[index] = value;
    setTimeResource(update);
  }

  const onRemoveTimeResource = (index) => {
    const update = timeResources.filter((_, id) => id !== index);
    setTimeResource(update);
  }
  ...
  const handleChangeKpis = (value, index) => {
    const update = [...kpiss];
    update[index] = value;
    setKpis(update);
  }

  const onRemoveKpis = (index) => {
    const update = kpiss.filter((_, id) => id !== index);
    setKpis(update);
  }

  const [state, formAction, isPending] = useActionState(projectAction, {
    errors: {},
  });
